...
from flask import Flask, render_template, g, request, send_from_directory, redirect, url_for, flash, jsonify, send_file
...
def format_datetime_filter(dt, format_type='full', show_tooltip=True):
    ...
    if show_tooltip and format_type != 'full':
        return Markup(f'<span title="{full_timestamp}" class="cursor-help">{display}</span>')
...
def dashboard():
    ...
    with db.get_connection() as conn:
        ...
        stats['most_active_hour'] = most_active_hour_row['hour'] if most_active_hour_row else None
        ...
        stats['most_popular_day'] = most_popular_day_row['day_of_week'] if most_popular_day_row else None
        stats['most_popular_day_name'] = day_names.get(stats['most_popular_day'], _('N/A')) if stats['most_popular_day'] is not None else _('N/A')
...
def species_detail(taxon_id):
    ...
    with db.get_connection() as conn:
        ...
        return render_template('species_detail.html', species=species, visits=visits, recent_crops=recent_crops)
...
def correct_visit_species(visit_id):
    ...
    if inaturalist_url:
        ...
        if not taxon_id_from_url:
            flash('Invalid iNaturalist URL. Please use format: https://www.inaturalist.org/taxa/12345', 'error')
        ...
        if not new_taxon_id:
            flash('Failed to fetch species from iNaturalist. Please try again.', 'error')
            return redirect(url_for('visit_detail', visit_id=visit_id))

        flash(f'Species fetched from iNaturalist (taxon_id: {new_taxon_id}) and visit updated!', 'success')
...
def trigger_task(task_type):
    ...
    from .scheduler import process_files_task, download_unifi_task, face_annotation_task, regenerate_thumbnails_task
    import threading
    ...
    def run_task():
        try:
            ...
            logger.error("manual_task_failed", extra={
                "task_type": task_type,
                "error": str(e)
            })
...
def labeling_stats():
    ...
    with db.get_connection() as conn:
        ...
        confirmation_rate = (stats['confirmed'] / reviewed_total
                           if reviewed_total > 0 else 0)
        stats = dict(stats)
    ...
    return render_template('labeling_stats.html',
                          stats=stats,
                          species_stats=species_stats_list)
